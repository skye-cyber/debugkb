...
export default function IssueFormPage({ mode = "create" }) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        const payload = normalizePayload(form); // strip empty fields if desired
        if (mode === "create") {
            createMutation.mutate(payload, {
                onSuccess: (created) => navigate(`/issues/${created.id}`),
                onError: (err) => setServerErrors(err.fieldErrors),
            });
        } else {
            updateMutation.mutate(payload, {
                onSuccess: (updated) => navigate(`/issues/${updated.id}`),
                onError: (err) => setServerErrors(err.fieldErrors),
            });
        }
    };
    ...
    return (
        <form onSubmit={handleSubmit}>
            {serverErrors.length > 0 && (
                <div className="mb-4 rounded-lg border border-sev-critical/30 bg-sev-critical/5 p-3 text-xs text-sev-critical">
